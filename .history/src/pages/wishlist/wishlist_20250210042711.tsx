...
import { addToWishlist, removeFromWishlist } from "../../redux/reducers/wishlist-reducer";
        ...
        <div className="w-[270px] sm:w-[220px] md:w-[240px] lg:w-[270px]">
            <div className="bg-[#f5f5f5] rounded-[4px] relative">
                <Link to={`/product-detail/${id}`}>
                    <img 
                        className="mb-[16px] w-full object-cover" 
                        src={thumbnail} 
                        alt="image" 
                    />
                </Link>
            ...
            <Link to={`/products/${id}`}>
                <h3 className="font-medium text-[16px] mb-[8px] sm:text-[14px] md:text-[15px] lg:text-[16px]">{title}</h3>
            </Link>
            <div className="flex gap-[10px]">
                <p className="font-medium text-[16px] text-[#db4444] sm:text-[14px] md:text-[15px] lg:text-[16px]">${price}</p>
                <p className="text-sm sm:text-xs md:text-sm lg:text-base">{rating}</p>
            </div>
        </div>
    );
};
