    ...
    return (
        <div className="w-[270px] sm:w-[220px] md:w-[240px]">
            <div className="bg-[#f5f5f5] rounded-[4px] relative">
                ...
                <button 
                    onClick={handleLike}
                    className={`absolute top-[17px] right-[17px] w-[30px] h-[30px] rounded-full flex items-center justify-center transition-all duration-200 ${
            ...
            <Link to={`/products/${id}`}>
                <h3 className="font-medium text-[16px] mb-[8px] sm:text-[14px] md:text-[15px]">{title}</h3>
            </Link>
            <div className="flex gap-[10px]">
                <p className="font-medium text-[16px] text-[#db4444] sm:text-[14px] md:text-[15px]">${price}</p>
